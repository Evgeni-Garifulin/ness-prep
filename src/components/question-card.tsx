...
// NoteModal используется редко (только когда пользователь жмёт WRITE NOTE
// или EDIT у заметки). Вынесена в отдельный чанк через next/dynamic —
// в основном бандле секционной страницы её больше нет.
const NoteModal = dynamic(
  () => import("@/components/note-modal").then((m) => m.NoteModal),
  { ssr: false },
);
